...
**Production-support and internal tooling:** An internal ASP.NET Core (.NET 8) Razor Pages production-support portal is built and maintained with SQL Server and SQLite persistence, role-gated approval workflows, audit logging, live configuration diffing, pre-change-review visibility, and integrated service-desk request tracking. The .NET 8 Blazor Web App using Interactive Server is a separate container migration proof of concept. Python APIs and workflow automation, plus AI-assisted knowledge tooling with engineer-review controls, remain separate operational tooling.
